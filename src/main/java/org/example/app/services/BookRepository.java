...
public class BookRepository implements ProjectRepository<Book>, ApplicationContextAware {
    ...
    public boolean removeItemsByRegex(String queryRegex) {
        int deletedCnt = 0;
        ...
        for (Book book : retreiveAll()) {
            ...
            if (authorMatcher.find() || titleMatcher.find() || sizeMatcher.find()) {
                removeItemById(book.getId());
                deletedCnt++;
            }
        }
        logger.info("remove books by regexp completed, count = " + deletedCnt);
        return deletedCnt > 0;
    }
